...
/*
 * Usamos un buffer variable para no tener siempre el maximo de 4MB
 * Dejamos variables globales para no tener que pasar siempre un ADT
 * Vamos a mover memoria solo cuando no tengamos espacio
 * pero no lo vamos a achicar a medida que sube, porque si necesito mucha
 * memoria en algun momento, es raro que necesite menos luego
 */
